...
function createNumberFieldState(id) {
    return {
        id,
        type: `number`,
...
function createArrayFieldState(id) {
    return {
        id,
        type: `array`
    };
}
function createFieldState(field) {
    let showAs = field.showAs;
    if (!showAs) {
        if (field.group === `spectral`)
            showAs = `float32`;
        showAs = `decimal`;
    }
    if (showAs === `float32`)
        return createArrayFieldState(field.id);
    return createNumberFieldState(field.id);
}
function updateNumber(s, v) {
    if (s.type === `number`) {
        if (Number.isNaN(v))
            return s;
...
function handleValue(v, field, fieldState) {
    if (!fieldState) {
    ...
    if (!sectionEl)
        return;
    if (sectionEl.classList.contains(`frozen`))
        return;
    if (typeof v === `number`) {
...
const formatFloat = (v) => v.toPrecision(2);
const formatInteger = (v) => Math.floor(v).toString();
function getElementsForField(field) {
...
function getStatsForField(fieldState, field) {
...
//   }
// }
function setupContainer(fields, parentElQuery) {
...
function setup() {
    ...
    document.querySelector(`#plotSpeed`)?.addEventListener(`change`, event => {
        const speed = Math.floor(event.target.valueAsNumber);
        for (const el of document.querySelectorAll(`ixfx-plot-element`)) {
...
//   }
// }
function* enumeratePerceptualFields() {
    for (const [key, value] of Object.entries(settings.fields)) {
        if (value.group === `perceptual`)
            yield value;
    }
}
function* enumerateTimeDomainFields() {
    for (const [key, value] of Object.entries(settings.fields)) {
        if (value.group === `time`)
            yield value;
    }
}
function* enumerateSpectralFields() {
    for (const [key, value] of Object.entries(settings.fields)) {
        if (value.group === `spectral`)
            yield value;
    }
}
setup();
