...
const settings = Object.freeze({
    inputLastEl: document.getElementById(`inputLast`),
...
function onMidiMessage(m) {
    ...
    if (!chkEvFilterNotes.checked && (m.command === `noteoff` || m.command === `noteon`))
        return;
    if (!chkEvFilterCc.checked && m.command === `cc`)
        return;
    if (!chkEvFilterPb.checked && m.command === `pitchbend`)
        return;
