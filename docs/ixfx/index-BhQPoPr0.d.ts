...
import { R as Rgb8Bit, a as Rgb, b as Colour } from './Colour-VMbCHzG9.js';
...
type CanvasRegionSpec = CanvasRegionSpecRelativePositioned | CanvasRegionSpecRelativeSized | CanvasRegionSpecMatched;
declare class CanvasSource {
    ...
    /**
     * Creates a fixed region in canvas-absolute points.
     * That is, whatever `canvasCoordsRect` is, this is always the region size, regardless
     * of changes to the source.
     *
     * See also: {@link createRelativePositioned}
     * @param canvasCoordsRect
     * @returns
     */
    createFixedAbsolute(canvasCoordsRect: RectPositioned): CanvasRegion;
    createRegion(spec: CanvasRegionSpec): CanvasRegion;
    createRelativeMatched(match: HTMLElement | string): CanvasRegion;
    /**
     * Creates & adds a relative canvas region
     *
     * scale:
     * * 'independent': 0..1 x & y values are treated independent. ie, y:1 will be the height of source, x:1 will be the width of source.
     *
     * See also: {@link createFixedAbsolute}
     * @param rect Rectangle
     * @param scale Relative coordinate style
     * @returns
     */
    createRelativePositioned(rect: RectPositioned, scale?: `independent`): CanvasRegion;
    createRelativeSized(rect: Rect, position: `center`, scale?: `independent`): CanvasRegion;
    clear(): void;
