import {
  Svg_exports
} from "./chunk-HGE7G7BI.js";
import {
  Video_exports
} from "./chunk-HLYAUPFJ.js";
import {
  clamp as clamp2
} from "./chunk-BRTBADUP.js";
import {
  Empty2,
  EmptyPositioned,
  ...
  isEqualSize,
  rect_exports
} from "./chunk-N3JSE3NS.js";
import {
  StackImmutable
} from "./chunk-FJTKYBBV.js";
import {
  immutable
} from "./chunk-ZNNNL3GQ.js";
import {
  Empty,
  Ops,
  guard2,
...
import {
  resolveEl,
  resolveElementTry
} from "./chunk-JTLBTJS3.js";
import {
  withoutUndefined
} from "./chunk-SAVGPB4Z.js";
import {
  resultErrorToString
} from "./chunk-MVMR5OYI.js";
import {
  throwArrayTest
...
var CanvasSource = class {
  ...
  createRegion(spec) {
    if (`relativePositioned` in spec) {
      return this.createRelativePositioned(spec.relativePositioned, spec.scale);
    }
    if (`relativeSize` in spec) {
      return this.createRelativeSized(spec.relativeSize, spec.position, spec.scale);
    }
    if (`match` in spec) {
      return this.createRelativeMatched(spec.match);
    }
    throw new Error(`Spec doesn't seem valid`);
  }
  createRelativeMatched(match) {
    const result = resolveElementTry(match);
    if (!result.success) {
   ...
   * @returns 
   */
  createRelativePositioned(rect2, scale = `independent`) {
    let compute;
    switch (scale) {
      case `independent`:
        compute = (source) => ({
        ...
        break;
      default:
        throw new Error(`Param 'kind' unknown (${scale})`);
    }
    return this.#add(new CanvasRegion(this, compute));
  }
  createRelativeSized(rect2, position, scale = `independent`) {
    let compute;
    switch (scale) {
      case `independent`:
        compute = (source) => {
          const width = rect2.width * source.width;
          const height = rect2.height * source.height;
          return {
            width,
            height,
...
//# sourceMappingURL=chunk-4EA2VEAG.js.map