...
const setupEnvelope = () => {
    ...
    const sampleEnvironment = async () => {
        ...
        try {
            ...
            console.log(o);
            o.shouldLoop = false;
            if (o.attackDuration > durationLimit)
                throw new Error(`attackDuration cannot be longer than ${durationLimit}ms.`);
            ...
            const data = await Async.toArray(Envelopes.adsrIterable({ env: o, sampleRateMs: sampleRate }));
            console.log(data);
    ...
    Forms.button(`#btnPasteEnv`, async () => {
        ...
        lines = lines.filter(l => {
            ...
            if (l.startsWith(`shouldLoop`))
                return false;
            if (l.startsWith(`sustainLevel`))
                return false;
            return true;
        });
