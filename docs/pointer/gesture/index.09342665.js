...
})({"7Dk5N":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "3513007e09342665";
"use strict";
...
},{}],"7pvp9":[function(require,module,exports) {
...
let state = Object.freeze({
    /** @type {TrackedPointMap} */ pointers: (0, _dataJs.pointsTracker)({
        storeIntermediate: false
    }),
    twoFinger: {
        rotation: (0, _dataJs.numberTracker)(),
        distance: (0, _dataJs.numberTracker)()
    },
    threeFinger: {
        area: (0, _dataJs.numberTracker)()
    },
    centroid: (0, _dataJs.pointTracker)(),
    /** @type number */ centroidAngle: 0
});
const gestureTwoFinger = (a, b)=>{
    ...
    const distanceAbs = (0, _geometryJs.Points).distance(a, b); // clamp(Points.distance(a, b) / maxDimension)
    twoFinger.distance.seen(distanceAbs);
    // Calculate rotation
    const rotationAbs = (0, _geometryJs.radianToDegree)((0, _geometryJs.Points).angle(a, b));
...
const gestureThreeFinger = (a, b, c)=>{
    ...
    const tri = (0, _geometryJs.Triangles).fromPoints([
        a,
        b,
        c
    ]);
    state.threeFinger.area.seen((0, _geometryJs.Triangles).area(tri));
};
/**
 * 
 ...
 */ const gestureCentroid = (pointers)=>{
    ...
    const centroid = (0, _geometryJs.Points).centroid(...pointers.last());
    state.centroid.seen(centroid);
    updateState({
        centroidAngle: (0, _geometryJs.radianToDegree)((0, _geometryJs.Points).angle(centroid, state.centroid.initial))
    });
};
const update = ()=>{
    ...
    const byAge = [
        ...pointers.trackedByAge()
    ];
    if (byAge.length >= 2) // Got at least two touches
    ...
    if (byAge.length >= 3) // Got at least three touches
    gestureThreeFinger(byAge[0].last, byAge[1].last, byAge[2].last);
    else state.threeFinger.area.reset();
    ...
    for (const v of byAge){
        ...
        displayMap.set(v.id, {
            ...
            angle: Math.round(latestPoint ? (0, _geometryJs.radianToDegree)((0, _geometryJs.Points).angle(latestPoint, v.initial)) : NaN)
        });
    }
    (0, _domJs.dataTableList)(`#pointers`, displayMap);
    // Update visuals
    draw();
};
const draw = ()=>{
    ...
    (0, _domJs.reconcileChildren)(currentPointsEl, pointers.store, (trackedPoint, el)=>{
        if (el === null) {
    ...
    (0, _domJs.reconcileChildren)(startPointsEl, pointers.store, (trackedPoint, el)=>{
        if (el === null) {
            el = document.createElement(`div`);
            el.innerText = trackedPoint.id;
        }
    ...
    if (centroid.initial === undefined) positionEl(startCentroidEl, {
        x: -1000,
        y: -1000
    }, thingSize);
    else positionEl(startCentroidEl, centroid.initial, thingSize);
    if (centroid.last === undefined) positionEl(centroidEl, {
        x: -1000,
        y: -1000
    }, thingSize);
    else positionEl(centroidEl, centroid.last, thingSize);
 ...
 */ const trackPoint = (ev)=>{
    ...
    pointers.seen(ev.pointerId.toString(), {
        x: ev.x,
        y: ev.y
    });
    update();
...
},{"ixfx/dist/geometry.js":"4cvN0","ixfx/dist/dom.js":"ao7hr","ixfx/dist/data.js":"jF4D4"}],"4cvN0":[function(require,module,exports) {
...
},{"./chunk-G5YZ6YRD.js":"igz1E","./chunk-4DC2CRSB.js":"jTrL9","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["7Dk5N","7pvp9"], "7pvp9", "parcelRequirefc40")

//# sourceMappingURL=index.09342665.js.map
