var _a;
import { fromNumbers, fromPoints, guard, guardNonZeroPoint, isEmpty, isNaN, isNull, isPlaceholder, isPoint, isPoint3d } from "./chunk-VUEFATQ3.js";
import { linearSpace, quantiseEvery } from "./chunk-647FAJGC.js";
import { dotProduct, minFast, minIndex } from "./chunk-NGZXMICH.js";
import { mutable } from "./chunk-FKXNN6PM.js";
import { sortByNumericProperty } from "./chunk-AGCCB7IA.js";
import { wrap } from "./chunk-W3MH7W5D.js";
import { scale } from "./chunk-56LYTHQU.js";
import { clamp, clampIndex } from "./chunk-ZJSCF2A4.js";
import { zipKeyValue } from "./chunk-UK6L3VJJ.js";
import { randomElement } from "./chunk-HKC65PTS.js";
import { defaultRandom } from "./chunk-5VWJ6TUI.js";
import { round } from "./chunk-S5D7YRXR.js";
import { throwIntegerTest, throwNumberTest, throwPercentTest } from "./chunk-CSXWZ3IC.js";
...
var centroid = (...points) => {
    ...
    const sum5 = points.reduce((previous, p) => {
        if (p === void 0)
            return previous;
        if (Array.isArray(p)) {
...
var arrow = (origin, from2, opts = {}) => {
    ...
    if (from2 === `tip`) {
        ...
        tailPoints = corners(fromTopLeft({ x: tri.a.x - tailLength, y: origin.y - tailThickness / 2 }, tailLength, tailThickness));
    }
        ...
        tailPoints = corners(fromTopLeft({ x: origin.x - midX, y: origin.y - midY }, tailLength + arrowSize, tailThickness));
    }
    else {
        tailPoints = corners(fromTopLeft({ x: origin.x, y: origin.y - tailThickness / 2 }, tailLength, tailThickness));
...
var normaliseByRect3 = (rect, normaliseByOrWidth, height4) => {
    ...
    if (height4 === void 0) {
        ...
        if (typeof normaliseByOrWidth === `number`) {
            ...
            throw new TypeError(`Expects rectangle or width and height parameters for normaliseBy`);
        }
    }
    return isPositioned(rect) ? Object.freeze({
        x: rect.x / width,
...
var offsetCardinals = (grid, start, steps, bounds = `stop`) => {
    ...
    const cells2 = directions.map((d, index) => offset(grid, start, vectors[index], bounds));
...
var array2dUpdater = (grid, array) => {
    const fn = (v, position) => {
        ...
        if (pos === void 0) {
            throw new Error(`Position does not exist. Pos: ${JSON.stringify(position)} Grid: ${JSON.stringify(grid)}`);
        }
        array[pos.y][pos.x] = v;
    };
    return fn;
...
var toLine = (arc) => fromPoints(point(arc, arc.startRadian), point(arc, arc.endRadian));
var point = (arc, angleRadian3, origin) => {
    if (angleRadian3 > arc.endRadian)
        throw new Error(`angleRadian beyond end angle of arc`);
    if (angleRadian3 < arc.startRadian)
...
var centroid2 = (t2) => {
    guard7(t2);
    const total = reduce([t2.a, t2.b, t2.c], (p, accumulator) => ({
        x: p.x + accumulator.x,
        y: p.y + accumulator.y
    }));
    const div = {
...
export { joinPointsToLines, isLine, length, guard3 as guard, isPlaceholder2 as isPlaceholder, getPointParameter2 as getPointParameter, distance, divide, degreeToRadian, radianInvert, radianToDegree, radiansFromAxisX, subtract, Empty, Polar_exports, isEqual, angleRadian, intersectsPoint, Placeholder, distanceToExterior, multiply2 as multiply, relation, point_exports, Empty2, line_exports, Waypoint_exports, corners, shape_exports, Layout_exports, circle_exports, cardinal, getEdgeX, getEdgeY, empty, emptyPositioned, isEqualSize, multiply4 as multiply2, subtract3 as subtract2, rect_exports, isQuadraticBezier, isCubicBezier, path_exports, Grid_exports, quadraticSimple, toPath3 as toPath, bezier_exports, CompoundPath_exports, Ellipse_exports, CurveSimplification_exports, QuadTree_exports, placeholder, placeholderPositioned, scaler, Scaler_exports, ImageDataGrid_exports, Convolve2d_exports, arc_exports, Vector_exports, SurfacePoints_exports, corners2, triangle_exports, geometry_exports };
