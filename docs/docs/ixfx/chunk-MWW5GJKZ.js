...
import { pairwise } from "./chunk-VJWZGNDD.js";
import { defaultRandom } from "./chunk-5VWJ6TUI.js";
import { throwNumberTest } from "./chunk-2OY2BTO2.js";
...
function fromCam16(cam162, env) {
    ...
    const a2 = r * cosh;
    const b2 = r * sinh;
    const rgb_c = unadapt(multiplyMatrices(m1, [p2, a2, b2]).map((c4) => {
        return c4 * 1 / 1403;
    }), env.fl);
    return multiplyMatrices(cat16Inv, rgb_c.map((c4, i) => {
...
function toCam16(xyzd65, env) {
    ...
    const rgbA = adapt$1(multiplyMatrices(cat16, xyz100).map((c4, i) => {
        return c4 * env.dRgb[i];
    }), env.fl);
    ...
    const et = 0.25 * (Math.cos(hRad + 2) + 3.8);
    const t = 5e4 / 13 * env.nc * env.ncb * zdiv(et * Math.sqrt(a2 ** 2 + b2 ** 2), rgbA[0] + rgbA[1] + 1.05 * rgbA[2] + 0.305);
...
var cam16 = new ColorSpace({
    ...
    toBase(cam162) {
        return fromCam16({ J: cam162[0], M: cam162[1], h: cam162[2] }, viewingConditions$1);
    }
});
...
function steps(c12, c22, options = {}) {
    ...
    let { maxDeltaE, deltaEMethod, steps: steps2 = 2, maxSteps = 1e3, ...rangeOptions } = options;
    if (!colorRange) {
        [c12, c22] = [getColor(c12), getColor(c22)];
...
var toHsl = (colour, safe = false) => {
    if (typeof colour === `string` && colour === `transparent`)
        return { h: 0, s: 0, l: 0, opacity: 0, space: `hsl` };
    const c4 = resolve(colour);
    const hsl = c4.hsl;
    let hue = hsl[0];
    if (Number.isNaN(hue) && safe)
        hue = 0;
    let sat = hsl[1];
    if (Number.isNaN(sat) && safe)
        sat = 0;
    const parsedHsl = {
        h: hue / 360,
        s: sat / 100,
...
var resolve = (colour) => {
    if (typeof colour === `string`) {
        ...
        if (isHsl(colour))
            return new Color(hslToColorJs(colour));
        if (isRgb(colour))
            return new Color(rgbToColorJs(colour));
        if (isOklch(colour))
            return new Color(oklchToColorJs(colour));
    }
    return colour;
};
...
var scale3 = (colours, numberOfSteps, opts = {}) => {
    ...
    const steps2 = pieces.map((piece) => piece[0].steps(piece[1], { space, hue, steps: stepsPerPair, outputSpace: `srgb` }));
    return steps2.flat();
};
...
var isHsl = (p2) => {
    ...
    if (typeof p2 !== `object`)
        return false;
    if (p2.spaceId !== void 0)
        return false;
    if (p2.coords !== void 0)
        return false;
    const space = p2.space;
    if (space !== `hsl` && space !== void 0)
        return false;
    const pp = p2;
    if (pp.h === void 0)
        return false;
...
var isRgb = (p2) => {
    ...
    if (typeof p2 !== `object`)
        return false;
    if (p2.space !== `srgb` && p2.space !== void 0)
        return false;
    if (p2.r === void 0)
        return false;
    if (p2.g === void 0)
...
export { scale, scaler, scaleClamped, scalePercentages, scalePercent, scalerPercent, toHsl, randomHue, resolve, resolveToString, toHex, opacity, getCssVariable, Colour_exports };
//# sourceMappingURL=chunk-MWW5GJKZ.js.map
//# sourceMappingURL=chunk-MWW5GJKZ.js.map