import { Rect } from "../../ixfx/geometry.js";
...
export const resample = (destinationContext: CanvasRenderingContext2D, options: Partial<ResampleOptions>) => {
  ...
  const drawBox = (context: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, size: Rect) => {
    context.beginPath();
  ...
  const run = (rotationRadians: number) => {
    if (offContext) {
